...
| `total_conditions` | Integer | Tổng số điều kiện (conditions) của quy tắc |
...
| `cond{sortOrder}_operator` | String | Toán tử so sánh của điều kiện có số thứ tự `sortOrder` (ví dụ: `>`, `<`, `=`,...) |
