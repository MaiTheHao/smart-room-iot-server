import { getAutomations, createAutomation, updateAutomation, deleteAutomation, toggleAutomationStatus, executeAutomationNow } from '../../../../api/automation.api.js';
...
export const JobModal = (() => {
	...
	const init = () => {
		...
		if (typeof flatpickr !== 'undefined') {
			fpTime = flatpickr(elements.timePicker, {
				enableTime: true,
				noCalendar: true,
				dateFormat: "H:i:S",
				enableSeconds: true,
	...
	const open = (data = null) => {
		...
		if (isEdit) {
			...
			if (fpTime) fpTime.setDate(parsedCron.timeStr);
			...
			elements.scheduleType.dispatchEvent(new Event('change'));
			if (fpTime) fpTime.setDate('00:00:00');
			elements.weeklyGroup.querySelectorAll('.form-check-input').forEach(cb => cb.checked = false);
	...
	const validateAndGetCron = () => {
		...
		const timeStr = elements.timePicker.value || '00:00:00';
		let daysOfWeek = [];
		let dayOfMonth = '1';
	...
	const submit = async (e, onRefresh) => {
		...
		try {
			...
			const [err, res] = isUpdate ? await updateAutomation(id, payload) : await createAutomation(payload);
