...
export const ActionModal = (() => {
    ...
    const loadDevices = async (roomId, category, selectedId = null) => {
        ...
        if (!roomId || !category) {
            elements.targetId.innerHTML = '<option value="" disabled selected>Please select room and category first</option>';
            return;
        }
        ...
        try {
            ...
            if (!err && res && res.data) {
                ...
                if (devices.length === 0) {
                    elements.targetId.innerHTML = '<option value="" disabled selected>No devices found</option>';
                } else {
                ...
                elements.targetId.innerHTML = '<option value="" disabled selected>Error loading devices</option>';
            }
        } catch (error) {
