...
let mainCharts = { temp: null, power: null };
...
export const RoomAnalytics = {
  ...
  initMainCharts() {
    ...
    if (tempEl && window.ApexCharts) {
      mainCharts.temp = new ApexCharts(tempEl, {
        ...commonOptions,
        series: [{ name: i18n.temp || 'Temperature', data: [] }],
        colors: ['#ef4444'],
      });
      mainCharts.temp.render();
    }
    if (powerEl && window.ApexCharts) {
      mainCharts.power = new ApexCharts(powerEl, {
        ...commonOptions,
        series: [{ name: i18n.power || 'Power', data: [] }],
        colors: ['#f59e0b'],
      });
      mainCharts.power.render();
    }
  },
...
export const DeviceRenderer = {
  ...
  renderOrUpdateAll(newDevices) {
    ...
    groups.forEach((g) => {
      ...
      if (newIds !== currentIds || pane.innerHTML.includes('text-center py-4')) {
        ...
        newItems.forEach((device) => this.updateCardStatus(device));
      }
    });
  },
  ...
  renderGroupFull(pane, items, emptyMsg) {
    ...
    const activeTabIds = Array.from(pane.querySelectorAll('.nav-link.active')).map(
      (t) => t.dataset.bsTarget,
    );
    ...
    pane.innerHTML =
      items.map((d) => this.renderCard(d)).join('') ||
      `<p class="text-muted text-center py-4">${emptyMsg}</p>`;
    ...
    activeTabIds.forEach((tid) => {
      ...
      if (tabBtn) {
        ...
        if (tabList)
          tabList.querySelectorAll('.nav-link').forEach((nl) => nl.classList.remove('active'));
      ...
      if (tabPane) {
        ...
        if (tabContent)
          tabContent
            ...
            .forEach((tp) => tp.classList.remove('show', 'active'));
  ...
  updateCardStatus(device) {
    ...
    const inputs = card.querySelectorAll(
      'input:not(.master-switch):not(.device-chart-range), button:not([data-bs-toggle]):not(.chart-type-group button)',
    );
...
export const DeviceChart = {
  async init(naturalId) {
    const container = document.querySelector(
      `.device-chart-container[data-natural-id="${naturalId}"]`,
    );
    ...
    if (window.flatpickr) {
      flatpickr(rangeInput, {
        ...
        altInput: true,
        altFormat: 'H:i',
        defaultDate: [new Date(initialChartState.range.from), new Date(initialChartState.range.to)],
        mode: 'range',
        onClose: (dates) => {
          if (dates.length === 2) {
