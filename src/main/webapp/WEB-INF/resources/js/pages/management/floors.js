import { getFloors, createFloor, updateFloor, deleteFloor, patchFloor } from '../../api/floor.api.js';
...
document.addEventListener('DOMContentLoaded', () => {
	...
	const MainForm = (() => {
		const elements = {
			modal: document.getElementById('floorModal'),
			form: document.getElementById('floorForm'),
			...
			submitBtn: document.getElementById('btnSubmitFloor'),
		};
		...
		const open = (data = null) => {
			...
			if (isEdit) {
				...
				elements.floorId.value = data.id || '';
			}
		...
		const validate = () => {
			...
			if (!Validator.level.isValidFormat(data.level)) {
				setError('level', i18n.valLevelInvalid);
			}
