class UserManager {
	...
	initDataTable() {
		this.table = $('#usersTable').DataTable({
			...
			columns: [
				...
				{
					data: 'lastLoginAt',
					render: (data) => (data ? `<small>${new Date(data).toLocaleString()}</small>` : '<span class="text-muted">Never</span>'),
				},
	...
	bindEvents() {
		...
		$('#usersTable tbody')
			...
			.on('click', '.btn-delete-all-device-controls', (e) => this.handleDeleteAllHardwareConfigs($(e.currentTarget).data('id')))
	...
	renderStatus(data) {
		return data === 'USER' ? '<span class="badge badge-user"><i class="fas fa-user mr-1"></i>User</span>' : '<span class="badge badge-gateway"><i class="fas fa-network-wired mr-1"></i>Gateway</span>';
	}
	...
	async handleSetupGateway($btn) {
		...
		const confirmed = await notify.confirm('Confirm Setup', `Are you sure you want to trigger setup configuration for gateway "${username}"?`, 'warning');
	...
	async handleDeleteAllHardwareConfigs(clientId) {
		const confirmed = await notify.confirmDelete('All Device Controls');
		if (!confirmed) return;

		try {
			await this.clientService.deleteAllHardwareConfigs(clientId);
	...
	#resolveSetupErrorMessage(raw) {
		...
		if (/ip address/i.test(raw)) return 'Gateway IP address not configured. Please update client information first.';
		if (/timed out/i.test(raw)) return 'Gateway connection timed out. Please check if gateway is online and network is accessible.';
		if (/not a hardware gateway/i.test(raw)) return 'This client is not a hardware gateway. Setup can only be triggered for gateway clients.';
		if (/connection refused|unreachable/i.test(raw)) return 'Cannot reach gateway. Please verify gateway IP address and ensure it is online.';
		if (/duplicate entry/i.test(raw)) return `Duplicate device detected. Some devices may already exist in the database.\n\nDetails: ${raw}`;
		if (/null id/i.test(raw)) return `Database persistence error (null ID). A previous failed setup may have left partial data. Try clearing room devices and retry.\n\nDetails: ${raw}`;
		if (/fantype|fan_type/i.test(raw)) return `Invalid or missing FanType in gateway response. The 'fanType' field must be set to "GPIO" or "IR" for fan devices.\n\nDetails: ${raw}`;
		if (/gpio_pin/i.test(raw)) return `Invalid GPIO pin value. Devices using BLUETOOTH or API control must include gpioPin set to 0.\n\nDetails: ${raw}`;
		if (/cannot be null|not.null/i.test(raw)) return `A required field is missing in gateway response data.\n\nDetails: ${raw}`;
		if (/persistence failed/i.test(raw)) return `Device persistence failed.\n\nDetails: ${raw}`;
		if (/invalid.*format|deserialization/i.test(raw)) return `Gateway returned data in an unexpected format. Please check gateway firmware and response structure.\n\nDetails: ${raw}`;
