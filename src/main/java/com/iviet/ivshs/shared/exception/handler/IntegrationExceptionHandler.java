...
public class IntegrationExceptionHandler {
        ...
        public ResponseEntity<ApiResponse<Void>> handleNetworkTimeout(Exception ex) {
                ...
                return ResponseEntity.status(HttpStatus.GATEWAY_TIMEOUT).body(ApiResponse.error(HttpStatus.GATEWAY_TIMEOUT, message));
        }
        ...
        public ResponseEntity<ApiResponse<Void>> handleExternalCall(RuntimeException ex) {
                ...
                return ResponseEntity.status(HttpStatus.BAD_GATEWAY).body(ApiResponse.error(HttpStatus.BAD_GATEWAY, ex.getMessage()));
        }
        ...
        public ResponseEntity<ApiResponse<Void>> handleRemoteNotFound(RemoteResourceNotFoundException ex) {
                ...
                return ResponseEntity.status(HttpStatus.NOT_FOUND).body(ApiResponse.error(HttpStatus.NOT_FOUND, "Requested resource does not exist on the partner system."));
        }
}
