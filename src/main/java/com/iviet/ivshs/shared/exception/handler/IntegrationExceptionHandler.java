...
public class IntegrationExceptionHandler {
    ...
    public ResponseEntity<ApiResponse<Void>> handleNetworkTimeout(Exception ex) {
        ...
        String message = ex instanceof NetworkTimeoutException ? ex.getMessage()
                : "Network timeout while connecting to external device or service.";
    ...
    public ResponseEntity<ApiResponse<Void>> handleRemoteNotFound(RemoteResourceNotFoundException ex) {
        ...
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body(ApiResponse.error(HttpStatus.NOT_FOUND,
                        "Requested resource does not exist on the partner system."));
    }
}
