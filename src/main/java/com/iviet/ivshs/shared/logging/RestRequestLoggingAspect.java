package com.iviet.ivshs.shared.logging;
...
public class RestRequestLoggingAspect {
    ...
    private static final String LOG_TYPE = "REST";
    private static final int MAX_LOG_LENGTH = 500;

    private final TraceLogger traceLogger;

    @Around("within(com.iviet.ivshs.controller.api..*)")
    public Object logAroundRest(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        log.info("Request started: type={}, id={}, method={}, uri={}, remote={}", LOG_TYPE, traceId, request.getMethod(), request.getRequestURI(), request.getRemoteAddr());
        log.debug("Request details: type={}, headers={}, params={}", LOG_TYPE, getHeadersInfo(request), request.getParameterMap());
        ...
        try {
            ...
            log.info("Request completed: type={}, id={}, status={}, duration={}ms, target={}", LOG_TYPE, traceId, status, duration, controllerMethod);

            if (log.isDebugEnabled()) {
                log.debug("Response details: type={}, result={}", LOG_TYPE, truncateResult(result));
            }

            traceLogger.logTrace(TraceLogger.TraceData.builder().traceId(traceId).scenarioId(MDC.get("scenarioId")).type(LOG_TYPE).method(request.getMethod()).uri(request.getRequestURI()).status(status).controller(controllerMethod)
                    .remote(request.getRemoteAddr()).startedAt(start).endedAt(endedAt).build());
    ...
    private String truncateResult(Object result) {
        ...
        return strResult.length() > MAX_LOG_LENGTH ? strResult.substring(0, MAX_LOG_LENGTH) + "... (truncated, total: " + strResult.length() + ")" : strResult;
    }
