...
public class FcmNotificationStrategy implements NotificationStrategy {
    ...
    public void send(NotificationRequest request) {
        List<String> fcmTokens = request.getRecipients().stream().flatMap(client -> client.getClientDevices().stream())
                .map(ClientDevice::getFcmToken).filter(token -> token != null && !token.isBlank()).distinct()
        ...
        log.info("[FCM] Dispatching push notification to {} tokens from {} recipients", fcmTokens.size(),
