...
public class TelemetryServiceImpl implements TelemetryService {
  ...
  private final TelemetryResponseProcessor telemetryResponseProcessor;
  ...
  public void takeByRoom(Long roomId) {
    List<ClientDto> gateways =
        clientService.getListGatewaysByRoomId(roomId, 0, 1000).content();
    ...
    try (var executor = Executors.newVirtualThreadPerTaskExecutor()) {
      List<CompletableFuture<Void>> futures = gateways.stream()
          .map(gateway -> CompletableFuture.runAsync(
              MdcTaskWrapper.wrap(() -> processTakeByGateway(gateway)), executor))
          .toList();

      CompletableFuture.allOf(futures.toArray(new CompletableFuture[0])).join();
      log.info(
          "Finished batch collection for [{}] in {}ms",
          identifier,
  ...
  protected void processTakeByGateway(ClientDto gateway) {
    try {
      ...
      GatewayFetchResult<TelemetryResponseDto> result =
          adapter.fetchGlobalTelemetry(gateway.ipAddress());

      if (!result.success()) {
        log.debug("Global telemetry skipped for gateway [{}]: {}", gateway.username(), result.message());
        return;
      }

      result.getData().ifPresent(data -> telemetryResponseProcessor.process(data, gateway.username()));
    } catch (Exception e) {
