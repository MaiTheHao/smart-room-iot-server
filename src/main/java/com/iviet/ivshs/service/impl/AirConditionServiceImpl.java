...
public class AirConditionServiceImpl implements AirConditionService {
	...
	public AirConditionDto update(Long id, UpdateAirConditionDto dto) {
			AirCondition ac = airConditionDao.findById(id).orElseThrow(() -> new NotFoundException("Air Condition not found with ID: " + id));
			...
			if (dto.roomId() != null && !dto.roomId().equals(ac.getRoom().getId())) {
					Room room = roomDao.findById(dto.roomId()).orElseThrow(() -> new NotFoundException("Room not found with ID: " + dto.roomId()));
					ac.setRoom(room);
			}

			if (dto.deviceControlId() != null) {
					DeviceControl dc = deviceControlDao.findById(dto.deviceControlId()).orElseThrow(() -> new NotFoundException("Device Control not found with ID: " + dto.deviceControlId()));
