...
public class IndexViewServiceImpl implements IndexViewService {
    ...
    public IndexViewModel getModel() {
        ...
        Map<Long, List<com.iviet.ivshs.dto.RoomDto>> floorRoomMap = (floors == null) ? Map.of()
                : floors.stream()
                ...
                : rooms.stream()
                        .collect(Collectors.toMap(
                                ...
                                room -> {
                                    ...
                                    var tempHistory = temperatureValueDao.getAverageHistoryByRoom(room.id(), startedAt,
                                            endedAt, divisor);
                                    Double lastestAvgTemperature = (tempHistory != null && !tempHistory.isEmpty())
                                            ? tempHistory.get(tempHistory.size() - 1).avgTempC()
                                            : null;

                                    var energyHistory = energyMetricDao.findHistory(EnergyMetricCategory.ROOM,
                                            room.id(), startedAt,
                                            endedAt, divisor);
