...
public class SmrcApplication implements WebApplicationInitializer {
  ...
  public void onStartup(ServletContext servletContext) throws ServletException {
    ...
    FilterRegistration.Dynamic encodingReg =
    ...
    FilterRegistration.Dynamic traceReg = servletContext.addFilter(
        "requestTraceFilter", new DelegatingFilterProxy("requestTraceFilter"));
    traceReg.addMappingForUrlPatterns(EnumSet.allOf(DispatcherType.class), false, "/*");

    // Cấu hình Security Filter
    FilterRegistration.Dynamic securityReg = servletContext.addFilter(
    ...
    FilterRegistration.Dynamic rateLimitReg = servletContext.addFilter(
        "rateLimitingFilter", new DelegatingFilterProxy("rateLimitingFilter"));
