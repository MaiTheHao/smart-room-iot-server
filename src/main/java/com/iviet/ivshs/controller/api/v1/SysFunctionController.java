...
public class SysFunctionController {
  ...
  @GetMapping
  public ResponseEntity<ApiResponse<PaginatedResponse<SysFunctionDto>>> getFunctions(@RequestParam(name = "page", defaultValue = "0")
  ...
  public ResponseEntity<ApiResponse<SysFunctionDto>> getFunctionById(@PathVariable(name = "functionId")
  Long functionId) {
    return ResponseEntity.ok(ApiResponse.ok(functionService.getById(functionId)));
  }

  @GetMapping("/code/{functionCode}")
  public ResponseEntity<ApiResponse<SysFunctionDto>> getFunctionByCode(@PathVariable(name = "functionCode")
  String functionCode) {
    return ResponseEntity.ok(ApiResponse.ok(functionService.getByCode(functionCode)));
  }

  @GetMapping("/with-group-status/{groupId}")
  public ResponseEntity<ApiResponse<List<SysFunctionWithGroupStatusDto>>> getFunctionsWithGroupStatus(@PathVariable(name = "groupId")
  Long groupId) {
  ...
  public ResponseEntity<ApiResponse<SysFunctionDto>> updateFunction(@PathVariable(name = "functionId")
  Long functionId, @RequestBody
  @Valid
  UpdateSysFunctionDto request) {
