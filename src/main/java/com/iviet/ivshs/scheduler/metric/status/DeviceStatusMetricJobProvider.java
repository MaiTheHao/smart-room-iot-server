package com.iviet.ivshs.scheduler.metric.status;
...
public class DeviceStatusMetricJobProvider implements MetricJobProvider {
    ...
    public List<MetricJobRegistration> getMetricJobs() {
        return List.of(MetricJobRegistration.builder().name(DeviceStatusMetricJob.JOB_NAME).group(DeviceStatusMetricJob.JOB_GROUP).jobClass(DeviceStatusMetricJob.class).intervalSeconds(engineProperties.getMetricStatusIntervalSeconds()).build());
    }
}
