package com.iviet.ivshs.scheduler;
...
public class TraceJobListener implements JobListener {
    ...
    public void jobWasExecuted(JobExecutionContext context, JobExecutionException jobException) {
        ...
        traceLogger.logTrace(
                TraceLogger.TraceData.builder().traceId(traceId).scenarioId(scenarioId).type("JOB").method("EXECUTE").uri("job://" + jobName).status(status).controller(jobClass).remote("localhost").startedAt(startTime).endedAt(endedAt).build());
