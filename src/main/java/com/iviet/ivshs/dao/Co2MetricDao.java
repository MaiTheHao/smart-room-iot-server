...
public class Co2MetricDao extends BaseEntityDao<Co2Metric> {
  ...
  public List<Co2Metric> findHistory(Long targetId, Instant from, Instant to) {
    ...
    return entityManager
        .createQuery(jpql, Co2Metric.class)
  ...
  public Optional<RoomCo2MetricDto> findLatestByRoomId(Long roomId) {
    return findLatestByRoomId(roomId, null);
  }

  public Optional<RoomCo2MetricDto> findLatestByRoomId(Long roomId, Instant threshold) {
    String jpql = """
        SELECT AVG(cs.currentCO2), MAX(cs.currentCO2)
        FROM Co2Sensor cs
        WHERE cs.room.id = :roomId
          AND cs.isActive = true
          AND cs.currentCO2 IS NOT NULL
          AND (:threshold IS NULL OR cs.updatedAt >= :threshold)
        """;
  ...
  public List<RoomCo2MetricDto> findHistoryByRoomId(
      Long roomId, Instant from, Instant to, int divisor) {
    String jpql = """
        SELECT
            (cm.unixMinute - MOD(cm.unixMinute, :divisor)) * 60L,
            AVG(cm.co2),
