...
public class HardwareConfigDao extends BaseEntityDao<HardwareConfig> {
    ...
    public boolean existsByClientId(Long clientId) {
        return count(
                root -> entityManager.getCriteriaBuilder().equal(root.get("client").get("id"), clientId)) > 0;
    }

    public boolean existsByRoomId(Long roomId) {
        return count(
                root -> entityManager.getCriteriaBuilder().equal(root.get("room").get("id"), roomId)) > 0;
    }
